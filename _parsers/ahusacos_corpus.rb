...
module AhusacosLite
  ...
  class Ahusacos
    ...
    def self.get_input
      begin
        cnumber = File.read("_script/iteration/value.txt").to_s.to_i
    ...
    def self.ahusaocs_saver
      ...
      a.db_filepath = "_data/language/conlang.nb"

      ## Definitely Sense
      a.train(:definitely_sense, "#{gender} #{noun} #{adjective} #{conjucation} #{verb} #{adverb}#{punctuation}", "word")

      ## Likely Sense
      a.train(:likely_sense, "#{gender} #{adjective} #{noun} #{conjucation} #{verb} #{adverb}#{punctuation}", "word")

      ## Ambiguous Sense
      a.train(:ambiguous_sense, "#{conjucation} #{verb} #{adverb}#{gender} #{noun} #{adjective}#{punctuation}", "word")
      a.train(:ambiguous_sense, "#{conjucation} #{verb} #{adverb}#{gender} #{adjective} #{noun}#{punctuation}", "word")
    ...
    def self.ahusacos_loader
      ...
      with_human_errors do
        a = NaiveBayes.load('_data/language/conlang.nb') 

        ## Spam
        b = File.read("_input/output.txt")

        result = a.classify(b)

        open("_bresult/result.txt", "w") { |f|
          f.puts result
        }
      end
    end
